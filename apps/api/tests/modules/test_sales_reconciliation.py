...
@pytest.mark.anyio
async def test_expire_to_reconcile(session):
    ...
    r2 = await client.post(f"/api/v1/sales/orders/{oid}/reconcile", json={"payment_id": pid}, headers=h)
    assert r2.status_code == 200
    assert r2.json()["TrangThai"] == "Chờ đối soát"

@pytest.mark.anyio
async def test_only_manager_resolve(session):
    d, t = await _setup(session)
    client = await _make_client(session)
    ...
    pay.status = "Hết hạn"
    await session.flush(); await session.commit()
    await client.post(f"/api/v1/sales/orders/{oid}/reconcile", json={"payment_id": pid}, headers=h)
    r2 = await client.post(f"/api/v1/sales/payments/{pid}/resolve", json={"outcome": "received"}, headers=h)
    assert r2.status_code == 403
    r3 = await client.post(f"/api/v1/sales/payments/{pid}/resolve", json={"outcome": "received"}, headers=hm)
    assert r3.status_code == 200

@pytest.mark.anyio
async def test_new_qr_refused_when_reconcile_pending(session):
    d, t = await _setup(session)
    client = await _make_client(session)
    h, _ = await _headers(session)
    oid = await _submit(session, client, h, d, t)
