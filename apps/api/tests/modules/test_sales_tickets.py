...
async def test_ticket_carries_info(session):
    ...
    r = await client.post("/api/v1/sales/orders", json={"MaBan": t.id, "lines": [{"MaMon": d.id, "SoLuong": 2, "GhiChu": "không hành"}]}, headers=h)
...
async def test_adding_dish_prints_additional_ticket(session):
    ...
    oid = r.json()["MaOrder"]
    # add another line
    r2 = await client.post(f"/api/v1/sales/orders/{oid}/lines", json={"dish_id": d.id, "quantity": 1}, headers=h)
