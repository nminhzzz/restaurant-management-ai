...
async def _setup_dish_with_stock(session, qty=10, need=1, dish_name="Món"):
    ...
    await session.flush()
    ing = Ingredient(
        name="NL_" + dish_name, unit="kg", min_stock=1, stock_qty=qty, is_deleted=False
    )
    session.add(ing)
    await session.flush()
...
async def _submit(session, client, headers, dish, table):
    r = await client.post(
        "/api/v1/sales/orders",
        json={"MaBan": table.id, "lines": [{"MaMon": dish.id, "SoLuong": 1}]},
        headers=headers,
    )
...
async def test_status_only_moves_forward(session):
    ...
    r = await client.patch(
        f"/api/v1/sales/orders/{oid}/lines/{lid}/status", json={"to": "Đã xác nhận xong"}, headers=h
    )
    assert r.status_code == 200
    r = await client.patch(
        f"/api/v1/sales/orders/{oid}/lines/{lid}/status", json={"to": "Đã phục vụ"}, headers=h
    )
    assert r.status_code == 200
    assert await line_status(session, lid) == "Đã phục vụ"
    r = await client.patch(
        f"/api/v1/sales/orders/{oid}/lines/{lid}/status", json={"to": "Chờ"}, headers=h
    )
...
async def test_only_manager_cancel_whole(session):
    ...
    r = await client.post(
        f"/api/v1/sales/orders/{oid}/cancel", json={"reason": "x"}, headers=h_cash
    )
    assert r.status_code == 403
    r = await client.post(
        f"/api/v1/sales/orders/{oid}/cancel", json={"reason": "khách bỏ về"}, headers=h_mgr
    )
    assert r.status_code == 200
