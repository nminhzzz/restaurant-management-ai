...
async def _cashier_headers(session):
    ...
    if u is None:
        u = User(username="cashier01", password_hash=hash_password("pass"), full_name="Cashier", role_id="CASHIER", status="Hoạt động")
        session.add(u)
        await session.flush()
...
async def _setup_dish_with_stock(session, stock_qty=10, dish_name="Món A", price=50000, need_per_unit=1):
    # group
    ...
    ing = Ingredient(name="NL_"+dish_name, unit="kg", min_stock=1, stock_qty=stock_qty, is_deleted=False)
    session.add(ing); await session.flush()
    bd = business_date.business_date_of(business_date.now())
    pv = DishPriceVersion(dish_id=d.id, price=price, business_date=bd, status=VersionStatus.HIEU_LUC.value, change_type="Tạo mới")
    ...
    gl = GoodsReceiptLine(receipt_id=gr.id, ingredient_id=ing.id, quantity=stock_qty, unit_price=1000)
    session.add(gl); await session.flush()
    lot = IngredientLot(ingredient_id=ing.id, receipt_line_id=gl.id, quantity_remaining=stock_qty, status="Còn hạn", received_at=business_date.now())
    session.add(lot); await session.flush()
    await session.commit()
    return d, ing, g
...
async def test_submit_takes_price_and_recipe(session):
    ...
    r = await client.post("/api/v1/sales/orders", json={"MaBan": t.id, "lines": [{"MaMon": d.id, "SoLuong": 2}]}, headers=headers)
...
async def test_display_code_counts(session):
    ...
    client = await _make_client(session)
    headers = await _cashier_headers(session)
    r1 = await client.post("/api/v1/sales/orders", json={"MaBan": t.id, "lines": [{"MaMon": d.id, "SoLuong": 1}]}, headers=headers)
    r2 = await client.post("/api/v1/sales/orders", json={"MaBan": t2.id, "lines": [{"MaMon": d.id, "SoLuong": 1}]}, headers=headers)
...
async def test_takeaway_no_table(session):
    ...
    client = await _make_client(session)
    headers = await _cashier_headers(session)
    r = await client.post("/api/v1/sales/orders", json={"LoaiDon": "Mang về", "lines": [{"MaMon": d.id, "SoLuong": 1}]}, headers=headers)
...
async def test_rejected_when_out_of_stock(session):
    ...
    session.add(ing2); await session.flush()
    d2 = Dish(name="Món hiếm", group_id=g.id, is_deleted=False)
    session.add(d2); await session.flush()
    bd2 = business_date.business_date_of(business_date.now())
    pv2 = DishPriceVersion(dish_id=d2.id, price=10000, business_date=bd2, status=VersionStatus.HIEU_LUC.value, change_type="Tạo mới")
    session.add(pv2); await session.flush()
    rc2 = Recipe(dish_id=d2.id, business_date=bd2, status=VersionStatus.HIEU_LUC.value, change_type="Tạo mới")
    session.add(rc2); await session.flush()
    ...
    session.add(gr2); await session.flush()
    gl2 = GoodsReceiptLine(receipt_id=gr2.id, ingredient_id=ing2.id, quantity=1, unit_price=5000)
    session.add(gl2); await session.flush()
    lot2 = IngredientLot(ingredient_id=ing2.id, receipt_line_id=gl2.id, quantity_remaining=1, status="Còn hạn", received_at=business_date.now())
    session.add(lot2); await session.flush(); await session.commit()

    t = await _setup_table(session)
    client = await _make_client(session)
    headers = await _cashier_headers(session)
    r = await client.post("/api/v1/sales/orders", json={"MaBan": t.id, "lines": [{"MaMon": d.id, "SoLuong": 1}, {"MaMon": d2.id, "SoLuong": 99}]}, headers=headers)
    assert r.status_code == 201
...
async def test_all_fail_returns_422(session):
    ...
    session.add(d); await session.flush()
    bd = business_date.business_date_of(business_date.now())
    pv = DishPriceVersion(dish_id=d.id, price=10000, business_date=bd, status=VersionStatus.HIEU_LUC.value, change_type="Tạo mới")
    session.add(pv); await session.flush()
    rc = Recipe(dish_id=d.id, business_date=bd, status=VersionStatus.HIEU_LUC.value, change_type="Tạo mới")
    session.add(rc); await session.flush()
    ...
    session.add(gl); await session.flush()
    lot = IngredientLot(ingredient_id=ing.id, receipt_line_id=gl.id, quantity_remaining=1, status="Còn hạn", received_at=business_date.now())
    session.add(lot); await session.flush(); await session.commit()
    t = DiningTable(name="Bàn X", is_deleted=False, status="Trống")
    session.add(t); await session.flush(); await session.commit()
    client = await _make_client(session)
    headers = await _cashier_headers(session)
    r = await client.post("/api/v1/sales/orders", json={"MaBan": t.id, "lines": [{"MaMon": d.id, "SoLuong": 99}]}, headers=headers)
...
async def test_note_stored(session):
    ...
    client = await _make_client(session)
    headers = await _cashier_headers(session)
    r = await client.post("/api/v1/sales/orders", json={"MaBan": t.id, "lines": [{"MaMon": d.id, "SoLuong": 1, "GhiChu": "không hành"}]}, headers=headers)
