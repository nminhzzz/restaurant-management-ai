...
export function HistoryPanel({
  ...
  const fetcher = useCallback(
    () =>
      apiFetch<{ items: SessionSummary[] }>(
        `/assistant/sessions?size=50&v=${refreshKey}`,
      ),
    [refreshKey],
  );
    ...
    <div className="flex h-full min-h-0 flex-col bg-surface">
      ...
      >
        ...
        {history.status === "error" && (
          <p className="p-2 text-danger-fg">{history.message}</p>
        )}
        ...
        {history.status === "ready" &&
          GROUPS.map((group) => {
            const items = history.data.items.filter(
              (s) => groupLabel(s.last_at, now) === group,
            );
            if (items.length === 0) return null;
            return (
              <div key={group}>
                <p className="px-2 pt-3 pb-1 text-[11px] font-bold tracking-wider text-subtle uppercase">
                  {group}
                </p>
                {items.map((s) => (
                  <button
                  ...
                  >
                    ...
                    <span className="text-xs text-subtle">
                      {s.turn_count} câu hỏi · {timeOf(s.last_at, group)}
                    </span>
                  </button>
                ))}
              </div>
