...
export function AssistantScreen() {
  ...
  return (
    <div className="flex h-[calc(100dvh-5.5rem)] overflow-hidden rounded-container border border-border bg-canvas lg:h-[calc(100dvh-3rem)]">
      <aside className="hidden w-72 shrink-0 border-r border-border xl:block">
        {history}
      </aside>
      {drawer && (
      ...
      <section className="flex min-w-0 flex-1 flex-col">
        ...
        {openError && (
          <p
            role="alert"
            className="mx-4 mt-3 rounded-container bg-danger-subtle px-3.5 py-2.5 text-danger-fg shadow-[inset_4px_0_0_0_var(--color-danger)]"
          >
            {openError}
          </p>
        )}
          ...
          <div className="px-5 pt-2 pb-3.5">
            <div className="mx-auto max-w-3xl">
              <Composer
                size="lg"
                disabled={pending}
                placeholder="Hỏi tiếp về số liệu…"
                onSubmit={(q) => void ask(q)}
              />
              <p className="mt-2 text-center text-xs text-subtle">
                Trợ lý có thể hiểu sai câu hỏi. Kiểm tra lại số liệu quan trọng
                ở mục Báo cáo.
              </p>
            </div>
          </div>
