        ...
        <TableHeader>
          <TableRow>
            {columns.map((c) => (
              <TableHead
                key={c.key}
                className={cn(isNumericKind(c.kind) && "text-right")}
              >
                {c.label}
              </TableHead>
            ))}
...
export function ResultTabs({ result }: { result: ChatResponse }) {
  ...
  if (result.chart && result.data.length > 0)
    tabs.push({ key: "chart", label: "Biểu đồ", Icon: ChartColumn });
  if (result.data.length > 0)
    tabs.push({ key: "table", label: "Bảng", Icon: Table2 });
  ...
  const current = tabs.some((t) => t.key === active)
    ? active
    : (tabs[0]?.key ?? null);
  ...
  return (
    <div className="overflow-hidden rounded-container border border-border bg-surface">
      <div
        role="tablist"
        aria-label="Kết quả"
        className="flex items-center gap-0.5 border-b border-border pr-3 pl-1"
      >
        {tabs.map(({ key, label, Icon }) => (
          <button
            ...
            className={cn(
              ...
              current === key &&
                "text-ink shadow-[inset_0_-2px_0_0_var(--color-ink)]",
            )}
          >
        ...
        {result.detail && (
          <span className="ml-auto text-xs whitespace-nowrap text-subtle tabular-nums">
            {result.detail.row_count} dòng ·{" "}
            {(result.detail.elapsed_ms / 1000).toLocaleString("vi-VN", {
        ...
        {current === "sql" && result.detail && (
          <div className="space-y-3">
            ...
            <p className="flex items-center gap-2 rounded-control bg-success-subtle px-2.5 py-2 text-[13px] font-semibold text-success-fg">
              ...
              Đã qua kiểm duyệt: chỉ đọc, đúng view {result.detail.view} của vai
              trò, có giới hạn số dòng.
            </p>
          </div>
        )}
      </div>
